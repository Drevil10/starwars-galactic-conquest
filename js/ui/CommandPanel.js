...
const CommandPanel = {
  ...
  position() {
    ...
    const mapPanelHeight =
      ...
      GameState.getState()?.turnPhase === 'command'
        ? 286
        : 254;

    this.root.style.top = `${12 + mapPanelHeight + 10}px`;
  },

  getPlanet() {
    if (!this.planetId || typeof CommandSystem === 'undefined') {
      return null;
    }
  ...
  button(label, action, disabled) {
    ...
    Object.assign(button.style, {
      ...
      borderRadius: '4px',
      background: disabled ? '#16213d' : '#163f71',
      color: disabled ? '#7886a8' : '#f0f7ff',
      fontWeight: '700',
      fontSize: '11px',
      letterSpacing: '0.2px',
  ...
  createHeader(commandPoints, defense) {
    ...
    stats.textContent = `PM: ${commandPoints} · DEF: ${defense}/5`;
  ...
  render(message = '', success = null) {
    ...
    const isPlayerPlanet = status === 'player';

    if (!planet || !isCommandPhase || !isPlayerPlanet) {
      this.root.style.display = 'none';
      return;
    }

    const commandPoints = CommandSystem.getCommandPoints();
    const defense = CommandSystem.getDefense(planet.id);
    const canSpend = commandPoints >= CommandSystem.actionCost;

    this.root.innerHTML = '';
    this.root.style.display = 'block';

    this.root.append(
      this.createHeader(commandPoints, defense)
    );
    ...
    if (!message) {
      if (defense >= CommandSystem.maxDefense) {
        message = 'Defensa planetaria al máximo.';
      } else if (!canSpend) {
        message = 'No quedan puntos de mando este turno.';
      } else {
        message = `Acciones disponibles para ${planet.name}.`;
      }
    }
    ...
    this.root.append(
      this.createMessage(message, color)
    );
  ...
  isCapital(planetId) {
    if (
      ...
      return TerritorySystem.getPlayerCapital() === planetId;
    }
  ...
  execute(method) {
    ...
    const result = CommandSystem[method](this.planetId);
